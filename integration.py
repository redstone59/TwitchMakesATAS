...
def run():
    ...
    os.system('"D:\\! various files\\python\\twitch bot doodads\\lua fceux.bat"')

def play(pause=-1, start=1, stop_movie=True):
    j=open("D:\\! various files\\python\\twitch bot doodads\\gizmo.lua").readlines()
    ...
    with open("D:\\! various files\\python\\twitch bot doodads\\gizmo.lua","w") as lua:
        lua.writelines(j)
    run()
